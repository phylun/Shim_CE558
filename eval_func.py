...
class eval_OD():
    def __init__(self):
        self.path = ['./prediction', '../test_annos']
        self.overlapRatio = 0.5
        self.class_map = {1: 'car', 2: 'truck', 3: 'bus', 4: 'motorcycle', 5: 'bicycle', 6: 'traffic_light', 7: 'person'}
        self.threshold = 0.5
        self.cls = len(self.class_map) + 1
        self.precision = True
        self.recall = True
        self.FPPIW = True
        self.roc = True
    ...
    def JaccardOverlap(self, bbox1, bbox2):
        ...
        if len(intersect_bbox) == 0:
            ...
            if intersect_width and intersect_height > 0:
                ...
                return float(intersect_size / float(bbox1_size + bbox2_size - intersect_size + Epslion))
            else:
                return 0
    ...
    def cumTpFp(self, gtFile, detFile, label, overlapRatio, file_format):
        ...
        if file_format[1] == '.txt':
            ...
            for gtLine in gtLines:
                # print(self.class_map[int(gtLine.split(' ')[0])])
                # print(str(label))
                # if gtLine.split(' ')[0] == str(label):
                if self.class_map[int(gtLine.split(' ')[0])] == str(label):
            ...
            rects_xml = self.parse_xml(gtFile)
            for rect_xml in rects_xml:
                if rect_xml[0] == str(label): # may be amended
        ...
        for detLine in detLines:
            ...
            if self.class_map[int(detLine.split(' ')[0])] == str(label):
        ...
        for indexDet, deti in enumerate(detRects):
            ...
            for indexGt, gti in enumerate(gtRects):
                iou = self.JaccardOverlap(detRects[indexDet], gtRects[indexGt])
                if iou > iou_max:
                    iou_max = iou
                    maxIndex = indexDet
                    blockIdx = indexGt
        ...
        for index, pair in enumerate(fp_copy):
            ...
            if fp_copy[index][0] > self.threshold:
                ...
                if fp_copy[index][1] == 1:  # false positive
                    fp_th += 1
        fppw = float(fp_th) / float(fp_th_num + Epslion)
        ...
        for index, pair in enumerate(tp_copy):
            ...
            if tp_copy[index][0] > self.threshold:
                ...
                if tp_copy[index][1] == 1:
                    tp_th += 1
        tp_precision = float(tp_th) / float(tp_th_num + Epslion)
    ...
    def computeAp(self, label):
        ...
        for i in range(num):
            prec.append(float(tp_cumsum[i]) / float(tp_cumsum[i] + fp_cumsum[i] + Epslion))
            rec.append(float(tp_cumsum[i]) / float(self.all_num_pos + Epslion))
            fpr.append(float(fp_cumsum[i]) / float(tp_cumsum[i] + fp_cumsum[i] + Epslion))
        ...
        if self.roc:
            ...
            plt.ylabel('recall')
            plt.draw()
            plt.savefig("class_{}_roc.jpg".format(label))
            plt.close()
        ...
        if self.pr:
            ...
            plt.draw()
            # plt.pause(5)  # 显示5秒
            plt.savefig("class_{}_pr.jpg".format(label))
            plt.close()

        fppi = 0
        if self.FPPIW:
            fppi = float(fp_th) / float(self.all_num_pos + Epslion)
        ...
        recall = float(tp_th) / float(self.all_num_pos + Epslion)
